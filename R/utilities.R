...
bootdpci <- function(x, y, est = onestep, nboot = NA, alpha = .05, plotit = TRUE, dif = TRUE, BA = FALSE, SR = TRUE, ...) {
  ...
  if (!okay) SR <- FALSE
  output <- rmmcppb(x, y,
    est = est, nboot = nboot, alpha = alpha, SR = SR,
    plotit = plotit, dif = dif, BA = BA, ...
  )$output
  list(output = output)
}
...
rmmcppb <- function(x, y = NULL, alpha = .05,
  ...
  if (dif) {
    ...
    temp <- rmmcppbd(x,
      y = y, alpha = .05, con = con, est, plotit = plotit, grp = grp, nboot = nboot,
      hoch = TRUE, ...
    )
    output <- temp$output
    con <- temp$con
...
rmmismcp <- function(x, y = NA, alpha = .05, con = 0, est = tmean, plotit = TRUE, grp = NA, nboot = 500,
  ...
  sigvec <- (test[temp2] >= zvec)
  output <- matrix(0, connum, 6)
  dimnames(output) <- list(NULL, c(
    "con.num", "psihat", "p.value",
  ...
  if (flag.con) {
    ...
    dimnames(test) <- list(NULL, c(
      "Group", "Group", "psi.hat", "p.value", "p.crit",
      "ci.low", "ci.upper"
    ))
    jcom <- 0
    for (j in 1:J) {
...
yuen.effect <- function(x, y, tr = .2, alpha = .05, plotit = FALSE,
  ...
  list(
    ci = c(low, up), p.value = yuen, dif = dif, se = sqrt(q1 + q2), teststat = test,
    crit = crit, df = df, Var.Explained = e.pow, Effect.Size = sqrt(e.pow)
  )
}
